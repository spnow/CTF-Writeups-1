...
long long crc()
{
	...
	for(int i = 0; i <= 48; i++ )
	{
		int index = (data[i]^ (des & 0xff)) & 0xff;
		des = ((des & 0xffffffffffffffff) >> 8);
		des ^= table[index];
	}
...
long long uncrc()
{
	long long des = 0x5416b1679db69fc3;
	for(int i = 0x71; i >= 49; i--)
	{
		int idx = ((des >> (8*7)) & 0xff);
		idx = findIdx(idx);
		des ^= table[idx];
		des = (des << 8) & 0xffffffffffffffff;
		des ^= (idx ^ (data[i]) & 0xff);
	}	
	return des;
}
		...
		for(int j = 0x20; j < 0x7f; j++)
		{
			for(int k = 0x20; k < 0x7f; k++)
			{
				for(int l = 0x20; l < 0x7f; l++)
				{
					...
					data[0x60] = k;
					data[0x70] = l;
					fprintf(f4, "%llx %x %x %x %x\n", crc(), i,j,k,l);
					fprintf(l4, "%llx %x %x %x %x\n", uncrc(), i,j,k,l);
				}
			}
		}
	}
